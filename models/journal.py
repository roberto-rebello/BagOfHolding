...
class Journal(db.Model):
    ...
    def __init__(self, author, title, is_private, **kwargs):
...
class Entry(db.Model):
    ...
    _image = db.Column(db.String(100), default = "")
    _journal = db.relationship('Journal', backref=db.backref('entries', lazy=True))

    _journal_id = db.Column(db.Integer, db.ForeignKey('journal.id'), nullable=False)
    _creation_date = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)


    def __init__(self, id, author, title, body, game_date, location, image, journal, **kwargs):
        super(Item, self).__init__(**kwargs)
        self._id = id
        self._author = author
        self._title = title
    ...
    def __str__(self):
        return "ID: {}\n\tTitle: {}\n\tAuthor: {}\n\tCreation: {}\n\tBody: {}\n\tDate: {}\n\tLocation: {}\n\t \
            Image: {}\n".format(str(self._id), self._title, self._author, str(self._creation_date), self._body,
            self._game_date, self._location, self._image)
